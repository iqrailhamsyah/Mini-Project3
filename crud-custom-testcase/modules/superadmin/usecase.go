...
func (uc UsecaseSuperadmin) CreateSuperadmin(superadmin SuperAdminParam) (*entities.Actor, error) {
	hasPass := middleware.HashPass(superadmin.Password)
...
func (uc UsecaseSuperadmin) LoginSuperadmin(id uint, username, password string) (*entities.Actor, string, error) {
	...
	comparePass := middleware.ComparePass([]byte(superAdmin.Password), []byte(password))
	if !comparePass {
		return nil, "", err
	}

	// Generate token JWT
	tokenString, err := middleware.GenerateToken(id, username)
	if err != nil {
		return nil, "", err
	}
